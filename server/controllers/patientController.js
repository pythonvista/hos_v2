...
async function getAll(req, res, next) {
  ...
  Patient.aggregate(aggregationStages)
    .exec(function (err, docs) { // The return is an array btw.
      ...
      const sortData = docs.sort((a, b) => new Date(b.user.createdAt) - new Date(a.user.createdAt));
      responseHandler(res, 200, { docs :sortData, c })
    });
}
...
async function getFirstTimePatients(req,res,next) {
  try {
    let firstTime = {}
