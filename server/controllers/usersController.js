...
const { escapeRegExp } = require('lodash'); // For regex sanitization

async function search(req, res, next) {
    try {
        // Validate inputs
        if (!req.user) {
            return responseHandler(res, 401, new Error('Unauthorized: User not authenticated'));
        }

        const { searchQuery, limit = '10', page = '0' } = req.query;
        if (!searchQuery || typeof searchQuery !== 'string' || searchQuery.trim() === '') {
            return responseHandler(res, 400, new Error('Invalid or missing search query'));
        }

        const parsedLimit = parseInt(limit, 10);
        const parsedPage = parseInt(page, 10);
        if (isNaN(parsedLimit) || parsedLimit <= 0 || isNaN(parsedPage) || parsedPage < 0) {
            return responseHandler(res, 400, new Error('Invalid limit or page'));
        }
        ...
        if (req.user.role === 'hospital') {
            ...
            return responseHandler(res, 400, new Error('Hospital not specified for user'));
        }

        // Sanitize search query for regex
        const sanitizedQuery = escapeRegExp(searchQuery.trim());
        ...
        const users = await User.find({
            ...
            .skip(parsedLimit * parsedPage)
            .limit(parsedLimit)
            .exec();

        console.log('-->', users, role, sanitizedQuery); // Debug log
