...
async function getByPatientId(req, res, next) {
  ...
  const parentId = req.params?.patient ? req.params.patient : req.params.patientID
  ...
  const getUserId = await Patient.findOne({ _id: parentId })
  query.patient = getUserId.user
...
async function get(req, res, next) {
  ...
  if (req.user.role === 'hospital') {
    ...
    if (fromDate !== null && toDate !== null) {
      ...
      query.createdAt = { "$lte": fromDate, "$gte": toDate };
    }
  ...
  // console.log(req.user);
  console.log(query, 95)
  try {
    Billing.find({ ...query, })
    // Billing.find({ ...query, paymentStatus: true })
      .sort({
        createdAt: -1
      })
      .skip(pageOptions.page * pageOptions.limit)
...
async function outstanding(req, res, next) {
  ...
  console.log(query, 95)
  try {
    Billing.find({ ...query, paymentStatus: false })
      .sort({
        createdAt: -1
      })
      .skip(pageOptions.page * pageOptions.limit)
