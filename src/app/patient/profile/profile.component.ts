...
export class ProfileComponent implements OnInit {
  isLoader = false;
  isNotSubscribed = false; // New flag for subscription status
  ...
  get(id) {
    ...
    this.patientService.getById(id).subscribe(
      (res: ResponseMessage) => {
        console.log(res);
        if (res.status == '200') {
          this.isNotSubscribed = false; // Reset flag
          this.patient = res.data;
          this.problems_PastMedicalHistoryValue =
            this.patient.problems_PastMedicalHistory;
          this.drugHistoryValue = this.patient.drugHistory;
          this.surgicalHistoryValue = this.patient.surgicalHistory;
          this.obstetrics_GynacologyHistoryValue =
            this.patient.obstetrics_GynacologyHistory;
          this.vaccinesValue = this.patient.vaccines;
          this.allergiesValue = this.patient.allergies;
          this.isLoader = false;
        } else if (res.status == '403') {
          // Handle not subscribed case
  ...
  markAsActive() {
    this.authService
      .toggleactive(this.patient.user._id)
  ...
  onSaveDiseaseChange(diseaseType, value) {
    ...
    this.patientService
      .updateFixedDisease(this.patient._id, { diseaseType, value })
      .subscribe((res: any) => {
        if (res.status === 200) {
